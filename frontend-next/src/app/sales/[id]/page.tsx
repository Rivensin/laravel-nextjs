  ...
  return (
    <div className="mt-4 px-2 pb-2">
      <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-6">
          ...
          <div className="flex lg:min-h-screen w-full items-center justify-center bg-gray-600 rounded-lg">
            <div className="w-80 bg-gray-50 px-4 pt-2 xl:pt-6 shadow-lg">
              <div className='text-xs flex justify-between pt-2'>
                ...
                <button type='submit' form='checkout-form' className='bg-green-400 px-2 hover:underline'>
                  Save Receipt
                </button>
              </div>
              <Image 
              ...
              <div className="flex flex-col gap-3 pb-6 pt-2 text-xs">
                <table className="w-full text-left">
                  <thead>
                    <tr className="flex justify-center items-center mb-2 underline">
                      <th className="flex-1 py-1">Product</th>
                      <th className="min-w-[44px] py-2 text-center">Qty</th>
                      <th className="min-w-[54px] py-2 text-center">Price</th>
                  ...
                  <tbody>
                      {Object.entries(quantities).filter(([id, qty]) => qty > 0).map(([id, qty]) => {
                        ...
                        return (
                          <tr className="flex justify-center items-center" key={id}>
                            <td className="flex-1 py-1">{product?.title}</td>
                            <td className="min-w-[44px] text-center">{qty}</td>
                            <td className="min-w-[54px] text-center">{product?.cost?.toLocaleString('id-ID')}</td>
                            <td className="min-w-[44px] text-center">{subtotal.toLocaleString('id-ID')}</td>
                          </tr>
                        )
                      })} 
                  </tbody>
        ...
        {/* Right Side: Table */}
        <div>
          <div className='grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-3'>
            {products.map(product => (
              <div key={product.id} className="border shadow-xs rounded-lg">
                <Image 
                ...
                <div className="text-center">
                  <h5 className="lg:h-10 xl:h-14 mb-2 mt-1 text-sm lg:text-md xl:text-lg font-semibold tracking-tight text-heading">
                    {product.title}
                  </h5>
                  <h5 className="mb-2 text-sm lg:text-md xl:text-lg tracking-tight text-heading">
                  ...
                  {product.id in quantities ? (
                    ...
                    <button 
                      onClick={() => {increase(product.id,20)}}
                      className='font-sans text-white rounded-b-lg bg-green-600 hover:bg-green-600/80 shadow-xs font-medium leading-5 text-sm py-2.5 w-full duration-700 ease-out transition-all'>
                      Add to Cart
                    </button>
                  )} 
                </div>
